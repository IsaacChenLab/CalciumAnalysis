...
if(nPCs>flims(2))
    errordlg('Number of PCs cannot exceed the number of frames. Please try again','Bad input','modal');
    return;
end
PCuse = str2num(answer{3});
if(PCuse(end)>flims(2))
     errordlg('Number of PCs cannot exceed the number of frames. Please try again','Bad input','modal');
    return;
end
mu = str2num(answer{4});
...
if(nIC>flims(2))
    errordlg('Number of ICs cannot exceed the number of frames. Please try again','Bad input','modal');
    return;
end
